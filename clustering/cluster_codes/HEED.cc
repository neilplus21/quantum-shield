...
void DoHeedRound(uint32_t r,
    ...
    // === HEED CH ELECTION ===
    // 1. Compute initial CH probability based on residual energy (normalized)
    ...
    double C_prob = 0.15;  // Initial CH probability (can tune 0.05-0.1)

    std::vector<double> chProb(NUM_NODES, 0);
    for (uint32_t i = 0; i < NUM_NODES; ++i) {
        if (nodeInfos[i].isDead) continue;
        // Normalized energy (avoid div0)
        double normE = (maxE > 0) ? (nodeInfos[i].energy / maxE) : 1.0;
        chProb[i] = std::min(1.0, C_prob * normE);
    }

    // 2. Main CH selection (1 iteration: probabilistic self-election)
    for (uint32_t i = 0; i < NUM_NODES; ++i) {
        if (nodeInfos[i].isDead) continue;
        double rnum = random->GetValue();
        if (rnum < chProb[i]) {
            nodeInfos[i].isCH = true;
            chSet.insert(i);
        }
    }
    // 3. If no CHs, force alive node with maxE to be CH
    if (chSet.empty()) {
        double maxEE = -1e9; int best = -1;
        for (uint32_t i = 0; i < NUM_NODES; ++i) {
            if (!nodeInfos[i].isDead && nodeInfos[i].energy > maxEE) {
                maxEE = nodeInfos[i].energy;
                best = i;
            }
        }
        if (best != -1) {
    ...
    // 4. Cluster formation (join closest CH within RADIUS)
    for (uint32_t i = 0; i < NUM_NODES; ++i) {
    ...
    // -- Assign orphans (outside RADIUS) to closest CH
    for (uint32_t i = 0; i < NUM_NODES; ++i) {
        if (nodeInfos[i].isDead || nodeInfos[i].isCH) continue;
    ...
    for (uint32_t i = 0; i < NUM_NODES; ++i) {
        if (nodeInfos[i].isDead) {
            ...
            anim->UpdateNodeColor(i, 0, 128, 0); // green for HEED CHs
    ...
    // -- Energy drain logic (same as LEACH/CHEF)
    ...
    for (auto ch : chSet) {
        ...
        double aggCost = E_ELEC * PKT_SIZE_BITS * members;  // or use E_DA
    ...
    // --- Metrics and log (same as before)
...
int main(int argc, char *argv[]) {
    ...
    AnimationInterface anim("heed-iot-anim.xml");
