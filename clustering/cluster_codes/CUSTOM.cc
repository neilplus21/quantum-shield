#include "ns3/core-module.h"
#include "ns3/network-module.h"
#include "ns3/internet-module.h"
#include "ns3/mobility-module.h"
#include "ns3/energy-module.h"
#include "ns3/netanim-module.h"
#include <vector>
#include <fstream>
#include <sstream>
#include <iostream>
#include <iomanip>
#include <set>
#include <cmath>
#include <string>
#include <algorithm>

using namespace ns3;

// --- CONFIGURATION ---
const uint32_t NUM_NODES = 20;
const uint32_t NUM_ROUNDS = 30;//dies at 69th round
const double RADIUS = 60.0;
const double INIT_ENERGY = 2.0;
const uint32_t PKT_SIZE = 4000;

// CSV Node setup
struct NodeInit { double x, y, energy, cpu; };

// Node Info for simulation
struct NodeInfo {
    Ptr<Node> node;
    Vector position;
    double energy;
    double cpu;
    bool isCH;
    int parentCH;
    bool isDead;
    uint32_t clusterId;
    int chCount; // Times this node has been CH
    NodeInfo() : node(0), position(), energy(0), cpu(0), isCH(false), parentCH(-1), isDead(false), clusterId(0), chCount(0) {}
};

std::vector<NodeInit> ReadNodeSetup(const std::string& filename) {
    std::vector<NodeInit> nodeInit;
    std::ifstream infile(filename);
    std::string line;
    std::getline(infile, line); // header
    while (std::getline(infile, line)) {
        std::stringstream ss(line);
        std::string token;
        NodeInit n;
        std::getline(ss, token, ','); n.x = std::stod(token);
        std::getline(ss, token, ','); n.y = std::stod(token);
        std::getline(ss, token, ','); n.energy = std::stod(token);
        std::getline(ss, token, ','); n.cpu = std::stod(token);
        nodeInit.push_back(n);
    }
    return nodeInit;
}

double Distance(const Vector& a, const Vector& b) {
    return std::sqrt(std::pow(a.x - b.x, 2) + std::pow(a.y - b.y, 2));
}

void DoCustomChefRound(uint32_t r,
                       std::vector<NodeInfo>* pNodeInfos,
                       AnimationInterface* anim,
                       std::ofstream* plogfile) {
    std::vector<NodeInfo>& nodeInfos = *pNodeInfos;
    std::ofstream& logfile = *plogfile;
    int N = nodeInfos.size();

    // --- Reset CHs and clusters for the round ---
    for (int i = 0; i < N; ++i) {
        nodeInfos[i].isCH = false;
        nodeInfos[i].parentCH = -1;
        nodeInfos[i].clusterId = 0;
    }

    // --- Dead node check ---
    for (int i = 0; i < N; ++i) {
        if (nodeInfos[i].energy <= 0.0) {
            nodeInfos[i].isDead = true;
            nodeInfos[i].energy = 0.0;
            anim->UpdateNodeColor(i, 128, 128, 128);
            anim->UpdateNodeDescription(i, "DEAD");
            anim->UpdateNodeSize(i, 7, 7);
        }
    }

    // --- Compute degree (connectivity) for each node ---
    std::vector<int> degree(N, 0);
    for (int i = 0; i < N; ++i) {
        if (nodeInfos[i].isDead) continue;
        for (int j = 0; j < N; ++j) {
            if (i != j && !nodeInfos[j].isDead && Distance(nodeInfos[i].position, nodeInfos[j].position) < RADIUS) {
                degree[i]++;
            }
        }
    }

    // --- Compute normalization factors ---
    double minE = 1e9, maxE = -1e9, minCPU = 1e9, maxCPU = -1e9;
    int minDeg = 1e9, maxDeg = -1e9, maxCH = 1;
    for (int i = 0; i < N; ++i) {
        if (nodeInfos[i].isDead) continue;
        minE = std::min(minE, nodeInfos[i].energy);
        maxE = std::max(maxE, nodeInfos[i].energy);
        minCPU = std::min(minCPU, nodeInfos[i].cpu);
        maxCPU = std::max(maxCPU, nodeInfos[i].cpu);
        minDeg = std::min(minDeg, degree[i]);
        maxDeg = std::max(maxDeg, degree[i]);
        maxCH = std::max(maxCH, nodeInfos[i].chCount);
    }

    // --- Compute custom score for each node ---
    std::vector<double> score(N, 0);
    for (int i = 0; i < N; ++i) {
        if (nodeInfos[i].isDead) continue;
        double normE = (maxE > minE) ? (nodeInfos[i].energy - minE) / (maxE - minE) : 1.0;
        double normCPU = (maxCPU > minCPU) ? (nodeInfos[i].cpu - minCPU) / (maxCPU - minCPU) : 1.0;
        double normDeg = (maxDeg > minDeg) ? (degree[i] - minDeg) / double(maxDeg - minDeg) : 1.0;
        double fairness = 1.0 - double(nodeInfos[i].chCount) / double(maxCH ? maxCH : 1); // Less if frequently CH

        // Weighted sum (tune as needed!)
        double wE = 0.35, wCPU = 0.35, wDeg = 0.2, wFair = 0.1;
        score[i] = wE * normE + wCPU * normCPU + wDeg * normDeg + wFair * fairness;
    }

    // --- CHEF-style local maxima CH selection ---
    std::set<int> chSet;
    for (int i = 0; i < N; ++i) {
        if (nodeInfos[i].isDead) continue;
        bool isHighest = true;
        for (int j = 0; j < N; ++j) {
            if (i == j || nodeInfos[j].isDead) continue;
            if (Distance(nodeInfos[i].position, nodeInfos[j].position) < RADIUS) {
                if (score[j] > score[i]) { isHighest = false; break; }
            }
        }
        if (isHighest) {
            nodeInfos[i].isCH = true;
            nodeInfos[i].chCount++;
            chSet.insert(i);
        } else {
            nodeInfos[i].isCH = false;
        }
    }

    // --- Fallback: if no CHs, pick global max ---
    if (chSet.empty()) {
        int best = -1; double bestScore = -1;
        for (int i = 0; i < N; ++i) {
            if (!nodeInfos[i].isDead && score[i] > bestScore) { bestScore = score[i]; best = i; }
        }
        if (best != -1) {
            nodeInfos[best].isCH = true;
            nodeInfos[best].chCount++;
            chSet.insert(best);
        }
    }

    // --- Assign cluster members ---
    for (int i = 0; i < N; ++i) {
        if (nodeInfos[i].isDead || nodeInfos[i].isCH) continue;
        double minDist = 1e9; int bestCH = -1;
        for (int ch : chSet) {
            double d = Distance(nodeInfos[i].position, nodeInfos[ch].position);
            if (d < minDist && d < RADIUS) { minDist = d; bestCH = ch; }
        }
        if (bestCH != -1) {
            nodeInfos[i].parentCH = bestCH;
            nodeInfos[i].clusterId = bestCH;
        } else {
            // Fallback: join closest CH even if outside radius
            for (int ch : chSet) {
                double d = Distance(nodeInfos[i].position, nodeInfos[ch].position);
                if (d < minDist) { minDist = d; bestCH = ch; }
            }
            nodeInfos[i].parentCH = bestCH;
            nodeInfos[i].clusterId = bestCH;
        }
    }

    // --- Visualization: Update color & label with CH ID or MemberOf ---
    for (int i = 0; i < N; ++i) {
        if (nodeInfos[i].isDead) {
            anim->UpdateNodeColor(i, 128, 128, 128);
            anim->UpdateNodeDescription(i, "DEAD");
            anim->UpdateNodeSize(i, 7, 7);
        } else if (nodeInfos[i].isCH) {
            anim->UpdateNodeColor(i, 0, 200, 255); // Cyan-CH for visual distinction
            anim->UpdateNodeDescription(i, "CH " + std::to_string(i));
            anim->UpdateNodeSize(i, 15, 15);
        } else {
            anim->UpdateNodeColor(i, 0, 0, 255);
            anim->UpdateNodeDescription(i, "M->" + std::to_string(nodeInfos[i].parentCH));
            anim->UpdateNodeSize(i, 7, 7);
        }
    }

    // --- Energy drain: Same as in LEACH ---
    double E_ELEC = 50e-9;
    double E_AMP = 100e-12;
    double PKT_SIZE_BITS = 4000 * 8;
    Vector baseStationPos(250, 250, 0);

    for (int i = 0; i < N; ++i) {
        if (!nodeInfos[i].isDead && !nodeInfos[i].isCH && nodeInfos[i].parentCH != -1) {
            double d = Distance(nodeInfos[i].position, nodeInfos[nodeInfos[i].parentCH].position);
            nodeInfos[i].energy -= (E_ELEC * PKT_SIZE_BITS + E_AMP * PKT_SIZE_BITS * d * d);
            nodeInfos[nodeInfos[i].parentCH].energy -= E_ELEC * PKT_SIZE_BITS;
        }
    }
    for (int ch : chSet) {
        int members = 0;
        for (int i = 0; i < N; ++i)
            if (!nodeInfos[i].isDead && nodeInfos[i].parentCH == ch) members++;
        double aggCost = E_ELEC * PKT_SIZE_BITS * members;
        double dToBS = Distance(nodeInfos[ch].position, baseStationPos);
        double txToBsCost = (E_ELEC * PKT_SIZE_BITS + E_AMP * PKT_SIZE_BITS * dToBS * dToBS);
        nodeInfos[ch].energy -= aggCost;
        nodeInfos[ch].energy -= txToBsCost;
    }

    // --- Metrics/logging ---
    double totalE = 0, minEnergy = 1e9, maxEnergy = -1e9;
    int numDead = 0;
    std::vector<int> clusterSizes(chSet.size(), 0);
    int chIdx = 0;
    std::map<int, int> chToIdx;
    for (int ch : chSet) chToIdx[ch] = chIdx++;
    for (int i = 0; i < N; ++i) {
        totalE += nodeInfos[i].energy;
        if (nodeInfos[i].energy < minEnergy) minEnergy = nodeInfos[i].energy;
        if (nodeInfos[i].energy > maxEnergy) maxEnergy = nodeInfos[i].energy;
        if (nodeInfos[i].isDead || nodeInfos[i].energy <= 0.0) numDead++;
        if (!nodeInfos[i].isDead && nodeInfos[i].parentCH != -1 && chToIdx.count(nodeInfos[i].parentCH))
            clusterSizes[chToIdx[nodeInfos[i].parentCH]]++;
    }
    double fairness = 0.0;
    if (!clusterSizes.empty()) {
        double meanCH = double(N - numDead) / clusterSizes.size();
        for (int size : clusterSizes) fairness += std::pow(size - meanCH, 2);
        fairness = std::sqrt(fairness / clusterSizes.size());
    }

    std::cout << "Round " << r
              << " Dead: " << numDead
              << " AvgE: " << totalE/N
              << " MinE: " << minEnergy
              << " MaxE: " << maxEnergy
              << " #CH: " << chSet.size()
              << " Fairness: " << fairness
              << std::endl;
    logfile << r << "," << numDead << "," << totalE/N << "," << minEnergy << "," << maxEnergy << "," << chSet.size() << "," << fairness << "\n";
}

int main(int argc, char *argv[]) {
    NodeContainer nodes;
    nodes.Create(NUM_NODES);

    std::vector<NodeInit> nodesetup = ReadNodeSetup("scratch/nodesetup.csv");
    if (nodesetup.size() != NUM_NODES) {
        std::cerr << "Error: node setup size (" << nodesetup.size()
                  << ") does not match NUM_NODES (" << NUM_NODES << ")." << std::endl;
        return 1;
    }

    BasicEnergySourceHelper energySourceHelper;
    ns3::energy::EnergySourceContainer sources;
    Ptr<ListPositionAllocator> posAlloc = CreateObject<ListPositionAllocator>();
    for (uint32_t i = 0; i < NUM_NODES; ++i) {
        posAlloc->Add(Vector(nodesetup[i].x, nodesetup[i].y, 0));
        NodeContainer singleNode;
        singleNode.Add(nodes.Get(i));
        energySourceHelper.Set("BasicEnergySourceInitialEnergyJ", DoubleValue(nodesetup[i].energy));
        auto nodeSource = energySourceHelper.Install(singleNode);
        sources.Add(nodeSource.Get(0));
    }

    MobilityHelper mobility;
    mobility.SetPositionAllocator(posAlloc);
    mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    mobility.Install(nodes);

    InternetStackHelper stack;
    stack.Install(nodes);

    AnimationInterface anim("custom-iot-anim.xml");
    std::vector<NodeInfo> nodeInfos(NUM_NODES);
    for (uint32_t i = 0; i < NUM_NODES; ++i) {
        nodeInfos[i].node = nodes.Get(i);
        nodeInfos[i].position = Vector(nodesetup[i].x, nodesetup[i].y, 0);
        nodeInfos[i].energy = nodesetup[i].energy;
        nodeInfos[i].cpu = nodesetup[i].cpu;
        nodeInfos[i].isCH = false;
        nodeInfos[i].parentCH = -1;
        nodeInfos[i].isDead = false;
        nodeInfos[i].clusterId = 0;
        nodeInfos[i].chCount = 0;
        anim.SetConstantPosition(nodes.Get(i), nodesetup[i].x, nodesetup[i].y);
        anim.UpdateNodeColor(i, 0, 255, 0);
        anim.UpdateNodeDescription(i, "Node " + std::to_string(i));
    }

    std::ofstream logfile("custom_log.csv");
    logfile << "Round,Dead,AvgEnergy,MinEnergy,MaxEnergy,CHs,Fairness\n";

    for (uint32_t r = 0; r < NUM_ROUNDS; ++r) {
        Simulator::Schedule(Seconds(r * 1.0), DoCustomChefRound, r, &nodeInfos, &anim, &logfile);
    }

    Simulator::Stop(Seconds(NUM_ROUNDS * 1.0 + 1.0));
    Simulator::Run();
    Simulator::Destroy();

    logfile.close();
    std::cout << "Simulation finished.\n";
    return 0;
}
